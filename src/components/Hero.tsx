    ...
    >
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full min-w-[177.77vh] min-h-[56.25vw] opacity-60">
          <iframe
            ...
            src="https://play.gumlet.io/embed/69fae51860e95a00ee85fa8c?background=true&autoplay=true&loop=true&disable_player_controls=true"
          ...
          <div className="lg:col-span-8 relative z-20">
            ...
            <motion.div style={{ x: textX }} className="relative">
              <motion.h1
                ...
                className="text-[18vw] lg:text-[14vw] font-display font-black leading-[0.75] tracking-tightest uppercase mix-blend-difference"
              >
                SNGO<br />
                <span className="text-white/40 italic font-serif font-light lowercase lg:ml-[1.5em] block">Creations</span>
              </motion.h1>
            </motion.div>
            ...
            >
              <div className="max-w-xs space-y-6">
                <p className="text-sm font-light text-white/50 leading-relaxed uppercase tracking-wider">
                  Orchestrating visual emotions through high-end VFX, motion design, and synthetic intelligence.
                </p>
                <div className="flex gap-4 p-1 glass rounded-full w-fit">
                   <button className="px-6 py-3 bg-white text-black text-[9px] font-black uppercase tracking-widest rounded-full hover:bg-white/90 transition-colors">
                     View Works
                   </button>
                   <button className="px-6 py-3 text-white text-[9px] font-black uppercase tracking-widest rounded-full hover:bg-white/10 transition-colors flex items-center gap-2">
      ...
      <div className="absolute bottom-0 left-0 w-full p-10 flex justify-between items-end">
        ...
        >
          {["Motion", "VFX", "AI"].map((tag) => (
            <div key={tag} className="flex items-center gap-4 group">
               <span className="text-[8px] font-black text-white/20 group-hover:text-white transition-colors">0{tag === "Motion" ? 1 : tag === "VFX" ? 2 : 3}</span>
               <span className="text-[10px] uppercase font-bold tracking-widest text-white/40 group-hover:text-white transition-colors">{tag}</span>
            </div>
          ))}
        </motion.div>
        ...
        >
           ...
           <span className="text-[8px] uppercase font-black text-white/20 tracking-[0.5em]">Explore</span>
        </motion.div>
      </div>
    </section>
  );
